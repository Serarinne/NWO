...
<head>
    ...
    <meta name="keywords" content="{{ $post->title }}, {{ $post->categories->pluck('name')->join(', ') }}, {{ env('APP_NAME') }}, neverness to everness blog, game wallpaper news">
                    ...
                    <div class="max-w-4xl mx-auto text-center">
                        <div class="flex justify-center gap-2 mb-6">
                            @foreach ($post->categories as $category)
                                <a href="{{ route('posts.category', $category) }}" class="px-3 py-1 text-xs font-bold text-white bg-cyan-600/80 backdrop-blur-md rounded-full shadow-lg hover:bg-cyan-500 transition-colors uppercase tracking-wider">
                                    {{ $category->name }}
                                </a>
                            @endforeach
                        </div>
                        ...
                        <div class="flex flex-wrap items-center justify-center gap-6 text-sm text-gray-300">
                            <a href="{{ route('users.wallpapers', $post->author) }}" class="flex items-center gap-2 hover:text-white transition-colors group">
                                <img src="{{ $post->author->user_image_webp }}" alt="{{ $post->author->name }}" class="w-8 h-8 rounded-full border border-gray-600 group-hover:border-cyan-400 transition-colors">
