  ...
  "@@graph": [
    {
      ...
      "name": "{{ $tag->name }} NTE Wallpapers",
      "description": "Browse {{ $tag->name }} NTE wallpapers and discover Neverness to Everness backgrounds, fanart, and related wallpaper collections.",
      "isPartOf": {
      ...
      "about": [
        ...
        {
          "@@type": "Thing",
          "name": "{{ $tag->name }} NTE Wallpaper"
        },
        {
          "@@type": "Thing",
          "name": "{{ $tag->name }} Neverness to Everness Wallpaper"
        }
      ],
      "primaryImageOfPage": {
    ...
    },
    {
      "@@type": "ImageObject",
      "@@id": "{{ route('tags.view', ['slug' => $tag]) }}#primaryimage",
      "name": "{{ $tag->name }} NTE wallpaper tag image",
            ...
            <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-12 relative z-10">
                <div class="flex flex-col md:flex-row items-center md:items-start gap-8">
                    ...
                    <div class="flex-shrink-0 relative group">
                        <div
                            class="w-32 h-32 md:w-48 md:h-48 rounded-2xl overflow-hidden border-4 border-gray-800 shadow-2xl relative z-10">
                            <picture>
                                <source srcset="{{ $tag->image['webp'] }}" type="image/webp">
                                <img src="{{ $tag->image['jpg'] }}" alt="{{ $tag->name }} NTE Wallpaper 4K Background"
                                    class="w-full h-full object-cover">
                            </picture>
                        </div>
                        <div
                            class="absolute inset-0 bg-pink-500 blur-xl opacity-20 group-hover:opacity-40 transition-opacity duration-500 -z-10 rounded-full">
                        </div>
                    </div>
                    ...
                    <div class="flex-grow text-center md:text-left pt-1 w-full md:w-auto">
                        <div class="flex items-center justify-center md:justify-start gap-2 mb-2">
                            <span
                                class="px-2 py-0.5 rounded bg-pink-900/50 text-pink-300 text-xs font-medium border border-pink-700/50">Tag</span>
                        </div>
                        <h1 class="text-4xl md:text-5xl font-extrabold text-white tracking-tight mb-4">
                            {{ $tag->name }} NTE Wallpapers</h1>
                        ...
                        @if (isset($relatedTags))
                            <div class="flex flex-wrap items-center justify-center md:justify-start gap-2 mb-5">
                                ...
                                @foreach ($relatedTags as $related)
                                    <a href="{{ route('tags.view', $related->slug) }}"
                                        ...
                                        <div class="w-5 h-5 rounded-full overflow-hidden flex-shrink-0 bg-gray-700">
                                            <img src="{{ $related->image['webp'] }}" alt="{{ $related->name }} NTE Wallpaper 4K Background"
                                                class="w-full h-full object-cover">
                                        </div>
                                        <span
                                            class="text-sm font-medium text-gray-300 group-hover:text-white truncate max-w-[120px]">{{ $related->name }}</span>
                                    </a>
                                @endforeach
                            </div>
                        @endif
        ...
        <div class="border-b border-gray-800 bg-gray-900/90 top-16 z-40 backdrop-blur-md shadow-md">
            <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-4">
                <div class="flex items-center gap-3">
                    <h2 class="text-lg font-bold text-white tracking-wide">All {{ $tag->name }} NTE Wallpapers</h2>
                    <span
                        ...
                        {{ number_format($tag->wallpapers_count) }}
                    </span>
                </div>
            </div>
        </div>
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            @else
                <div class="w-full py-20 text-center bg-gray-800/30 rounded-2xl border border-gray-700/50 border-dashed">
                    ...
                    <a href="{{ route('tags.index') }}" class="inline-block mt-4 text-cyan-400 hover:text-cyan-300 hover:underline">
                        Explore other tags
                    </a>
                </div>
            @endif
